...
def model_setting_codegpt(args):
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"] = "0"
    
    pretrain_model = "microsoft/CodeGPT-small-java-adaptedGPT2"
    config = GPT2Config.from_pretrained(pretrain_model)
    config.num_labels = 2
    tokenizer = GPT2Tokenizer.from_pretrained(pretrain_model)
    tokenizer.pad_token = '<pad>'
    config.pad_token_id = 50261
    encoder = GPT2ForSequenceClassification.from_pretrained(pretrain_model, config=config)   
    
    model = CodeGPT(encoder, config, tokenizer)
...
def detect_codegpt(snippet_dir, model, tokenizer, args):
    code_list = []
