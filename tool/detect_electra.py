...
from transformers import RobertaConfig, ElectraTokenizer, ElectraForSequenceClassification
...
def model_setting_electra(args):
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"] = "0"
    
    pretrain_model = "google/electra-base-discriminator"
    config = RobertaConfig.from_pretrained(pretrain_model)
    config.num_labels = 2
    tokenizer = ElectraTokenizer.from_pretrained(pretrain_model)
    encoder = ElectraForSequenceClassification.from_pretrained(pretrain_model, config=config)  
    
    model = ELECTRA(encoder, config, tokenizer)
...
def detect_electra(snippet_dir, model, tokenizer, args):
    code_list = []
